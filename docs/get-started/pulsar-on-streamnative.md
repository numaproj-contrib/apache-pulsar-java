...
Ensure you have Numaflow installed. If not, see the [Quick Start guide](https://numaflow.numaproj.io/quick-start/).
...
1. Go to [streamnative.io](https://streamnative.io/)  
...
3. Follow the guided prompts to create an organization  
   ![img.png](img.png)  
4. Follow steps 2–4 in this document: [StreamNative Quickstart Console](https://docs.streamnative.io/docs/quickstart-console)  
...
5. Select an instance, tenant, and namespace in the top-left bar. This will allow you to select "Topics" from the left side of the page:  
   ![img_7.png](img_7.png)  
6. To test producing messages, we will test the UDSink. Go to the [Sink API Key manifests](../sink/byte-array/manifests/api-key/api-key.md), copy these template yaml files, we will be providing the corresponding service URL and topic name.  
...
12. To see the messages produced, you can use the CLI or apply another pipeline for testing. Provide values for the required fields in the UDSource ConfigMap, which can be found at the [Source API Key manifests](../source/byte-array/manifests/api-key/api-key.md).  
...
13. **Avro schema validation (optional)**  
    If you have an Avro schema registered on your topic, you can enable schema validation. See the [Pulsar schema docs](https://pulsar.apache.org/docs/4.0.x/schema-overview/) for more on schemas.
